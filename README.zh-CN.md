...
| [fd](https://github.com/sharkdp/fd) | LeaderF 文件搜索后端 | 是 |
| [ripgrep (rg)](https://github.com/BurntSushi/ripgrep) | ctrlsf 代码搜索后端 | 是 |
...
推荐安装 [Nerd Font](https://github.com/ryanoasis/nerd-fonts)（如 Hack Nerd Font），以正常显示状态栏中的 Powerline 图标。
...
| Rust | rust-analyzer | `rustup component add rust-analyzer` 或 `brew install rust-analyzer` |
...
kmscon 使用系统内建的字体渲染器。如需在状态栏正确显示 Powerline 图标，请确保安装了 Nerd Font：

```bash
# 下载并安装 Hack Nerd Font 到系统目录
