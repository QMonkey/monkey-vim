...
F3      打开底部终端（预填 :botright terminal ++rows=20，输入命令后回车；直接回车打开空终端）
F4      切换全局终端（底部，打开/隐藏）
F5      切换全局终端（右侧，打开/隐藏）
```
...
自定义标记（!@#$%^&*()）已禁用：`m[0-9]` 回退为 Vim 原生 mark，相关映射
（`m<BS>`、`m?`、`]-`、`[-`、`]=`、`[=`）均已解除。
...
```text
F3      在底部新终端中运行命令（预填 :botright terminal ++rows=20，输入命令后回车；直接回车打开空终端）
F4      切换全局终端（底部，20 行）
F5      切换全局终端（右侧，半宽）
```
...
操作符与文本对象（§1.23）或 motion 组合使用：`{操作符}{文本对象}`。以下操作符均支持 `[count]`；标注了"嵌套"的操作符，`[count]` 表示"第 N 层包围结构"。
...
#### 1.23 文本对象（targets.vim / vim-indent-object / vim-sandwich / vim-matchup）
